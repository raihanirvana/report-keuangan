import { StyleSheet } from 'react-native';

import { colors, typography } from '../../Theme';

const styles = StyleSheet.create({
  confirmButton: {
    alignItems: 'center',
    backgroundColor: colors.slate900,
    borderRadius: 18,
    marginTop: 28,
    paddingVertical: 18,
    shadowColor: colors.slate900,
    shadowOffset: { height: 12, width: 0 },
    shadowOpacity: 0.18,
    shadowRadius: 16,
  },
  confirmButtonDisabled: {
    opacity: 0.55,
  },
  confirmButtonText: {
    color: colors.white,
    fontFamily: typography.fontFamily,
    fontSize: 15,
    fontWeight: '800',
  },
  container: {
    backgroundColor: colors.backgroundLight,
    flex: 1,
  },
  floatingAddButton: {
    alignItems: 'center',
    backgroundColor: colors.primary,
    borderColor: colors.backgroundLight,
    borderRadius: 34,
    borderWidth: 4,
    bottom: 22,
    height: 68,
    justifyContent: 'center',
    position: 'absolute',
    right: 24,
    shadowColor: colors.primary,
    shadowOffset: { height: 8, width: 0 },
    shadowOpacity: 0.36,
    shadowRadius: 14,
    width: 68,
  },
  floatingAddText: {
    color: colors.white,
    fontSize: 38,
    lineHeight: 42,
  },
  mainContent: {
    flex: 1,
  },
  pageContent: {
    flexGrow: 1,
    paddingBottom: 120,
  },
  pageScroll: {
    flex: 1,
  },
  periodContent: {
    gap: 16,
  },
  periodCreateBox: {
    backgroundColor: 'rgba(255, 204, 213, 0.12)',
    borderColor: 'rgba(255, 204, 213, 0.32)',
    borderRadius: 22,
    borderWidth: 1,
    gap: 12,
    padding: 14,
  },
  periodCreateButton: {
    alignItems: 'center',
    borderColor: 'rgba(238, 43, 108, 0.28)',
    borderRadius: 16,
    borderStyle: 'dashed',
    borderWidth: 2,
    paddingVertical: 13,
  },
  periodCreateButtonText: {
    color: colors.primary,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '800',
  },
  periodDateInput: {
    flex: 1,
  },
  periodDateButton: {
    backgroundColor: colors.white,
    borderColor: 'rgba(255, 204, 213, 0.56)',
    borderRadius: 16,
    borderWidth: 1,
    gap: 4,
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  periodDateDay: {
    alignItems: 'center',
    borderRadius: 12,
    height: 38,
    justifyContent: 'center',
    width: '13%',
  },
  periodDateDayActive: {
    backgroundColor: colors.primary,
  },
  periodDateDayText: {
    color: colors.slate600,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '800',
  },
  periodDateDayTextActive: {
    color: colors.white,
  },
  periodDateGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 4,
    justifyContent: 'space-between',
  },
  periodDateLabel: {
    color: colors.slate400,
    fontFamily: typography.fontFamily,
    fontSize: 10,
    fontWeight: '800',
    textTransform: 'uppercase',
  },
  periodDatePickerHeader: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  periodDatePickerNav: {
    alignItems: 'center',
    backgroundColor: colors.surface,
    borderRadius: 14,
    height: 36,
    justifyContent: 'center',
    width: 36,
  },
  periodDatePickerNavText: {
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 20,
    fontWeight: '800',
  },
  periodDatePickerPanel: {
    backgroundColor: colors.white,
    borderColor: 'rgba(255, 204, 213, 0.48)',
    borderRadius: 18,
    borderWidth: 1,
    gap: 10,
    padding: 12,
  },
  periodDatePickerTitle: {
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '800',
  },
  periodDateRow: {
    flexDirection: 'row',
    gap: 10,
  },
  periodDateValue: {
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '800',
  },
  periodDateWeekRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  periodDateWeekText: {
    color: colors.slate400,
    fontFamily: typography.fontFamily,
    fontSize: 10,
    fontWeight: '800',
    textAlign: 'center',
    width: '13%',
  },
  periodEmptyText: {
    color: colors.slate400,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '700',
    textAlign: 'center',
  },
  periodErrorText: {
    color: colors.primary,
    fontFamily: typography.fontFamily,
    fontSize: 11,
    fontWeight: '800',
  },
  periodGroupTitle: {
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '800',
    marginLeft: 4,
  },
  periodOption: {
    alignItems: 'center',
    backgroundColor: 'rgba(255, 204, 213, 0.14)',
    borderColor: 'rgba(255, 204, 213, 0.32)',
    borderRadius: 16,
    borderWidth: 1,
    flexGrow: 1,
    paddingHorizontal: 12,
    paddingVertical: 12,
  },
  periodOptionActive: {
    backgroundColor: colors.primary,
    borderColor: colors.primary,
  },
  periodList: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 10,
    marginBottom: 8,
  },
  periodActionButton: {
    backgroundColor: colors.white,
    borderRadius: 12,
    paddingHorizontal: 10,
    paddingVertical: 7,
  },
  periodActionDangerText: {
    color: colors.primary,
    fontFamily: typography.fontFamily,
    fontSize: 11,
    fontWeight: '800',
  },
  periodActionRow: {
    flexDirection: 'row',
    gap: 8,
  },
  periodActionText: {
    color: colors.slate600,
    fontFamily: typography.fontFamily,
    fontSize: 11,
    fontWeight: '800',
  },
  periodCancelText: {
    color: colors.primary,
    fontFamily: typography.fontFamily,
    fontSize: 12,
    fontWeight: '800',
  },
  periodCard: {
    backgroundColor: 'rgba(255, 204, 213, 0.14)',
    borderColor: 'rgba(255, 204, 213, 0.32)',
    borderRadius: 16,
    borderWidth: 1,
    flexGrow: 1,
    gap: 10,
    padding: 12,
  },
  periodCardActive: {
    backgroundColor: colors.primary,
    borderColor: colors.primary,
  },
  periodCardMain: {
    gap: 4,
  },
  periodFormTitleRow: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  periodInput: {
    backgroundColor: colors.white,
    borderColor: 'rgba(255, 204, 213, 0.56)',
    borderRadius: 16,
    borderWidth: 1,
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 13,
    fontWeight: '700',
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  periodOptionMeta: {
    color: colors.slate400,
    fontFamily: typography.fontFamily,
    fontSize: 10,
    fontWeight: '700',
    marginTop: 4,
  },
  periodOptionMetaActive: {
    color: colors.white,
  },
  periodOptionText: {
    color: colors.slate600,
    fontFamily: typography.fontFamily,
    fontSize: 12,
    fontWeight: '800',
  },
  periodOptionTextActive: {
    color: colors.white,
  },
  periodSheetContainer: {
    backgroundColor: colors.white,
    borderTopLeftRadius: 32,
    borderTopRightRadius: 32,
    paddingBottom: 32,
    paddingHorizontal: 24,
    paddingTop: 12,
    width: '100%',
  },
  reloadButton: {
    backgroundColor: colors.slate900,
    borderRadius: 20,
    color: colors.white,
    fontFamily: typography.fontFamily,
    fontSize: 14,
    fontWeight: '800',
    marginTop: 22,
    overflow: 'hidden',
    paddingHorizontal: 26,
    paddingVertical: 14,
    textAlign: 'center',
  },
  reloadIcon: {
    color: colors.primary,
    fontSize: 46,
    fontWeight: '800',
    marginBottom: 12,
  },
  reloadState: {
    backgroundColor: colors.backgroundLight,
    flexGrow: 1,
  },
  reloadTapArea: {
    alignItems: 'center',
    backgroundColor: colors.backgroundLight,
    flex: 1,
    justifyContent: 'center',
    paddingHorizontal: 34,
  },
  reloadText: {
    color: colors.slate600,
    fontFamily: typography.fontFamily,
    fontSize: 14,
    fontWeight: '600',
    lineHeight: 22,
    marginTop: 10,
    textAlign: 'center',
  },
  reloadTitle: {
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 22,
    fontWeight: '800',
    textAlign: 'center',
  },
  sheetBackButton: {
    alignItems: 'center',
    backgroundColor: '#F1F5F9',
    borderRadius: 18,
    height: 36,
    justifyContent: 'center',
    marginRight: 12,
    width: 36,
  },
  sheetBackText: {
    color: colors.slate600,
    fontSize: 30,
    fontWeight: '800',
    lineHeight: 32,
  },
  sheetCloseButton: {
    alignItems: 'center',
    backgroundColor: '#F1F5F9',
    borderRadius: 18,
    height: 36,
    justifyContent: 'center',
    width: 36,
  },
  sheetCloseText: {
    color: colors.slate400,
    fontSize: 26,
    lineHeight: 28,
  },
  sheetHandle: {
    alignSelf: 'center',
    backgroundColor: colors.accentPink,
    borderRadius: 4,
    height: 8,
    marginBottom: 24,
    width: 72,
  },
  sheetHeader: {
    alignItems: 'center',
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 24,
  },
  sheetTitle: {
    color: colors.slate900,
    fontFamily: typography.fontFamily,
    fontSize: 22,
    fontWeight: '800',
  },
  sheetTitleArea: {
    flex: 1,
  },
});

export default styles;
